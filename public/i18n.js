...
const I18N = {
  ...
  // ===== 2026-08-27 🧠 LLM 门户 =====
  'LLM 门户：选择增强/翻译用的大模型通道 — Hermes 本机零 Key / Ollama / LM Studio / OpenRouter / 任意 OpenAI 兼容端点': ['LLM Portal: pick the model channel for enhance/translate — local Hermes (zero keys) / Ollama / LM Studio / OpenRouter / any OpenAI-compatible endpoint', 'LLM ポータル：強化/翻訳用モデル経路 — ローカル Hermes（キー不要）/ Ollama / LM Studio / OpenRouter / 任意の OpenAI 互換'],
  '— 供 ✨增强 / 出站翻译 / 剧本解析使用的大模型通道': ['— the model channel behind ✨enhance / outbound translation / script analysis', '— ✨強化 / 送信翻訳 / 脚本解析が使うモデル経路'],
  '当前通道': ['Channel', '経路'],
  '自动（Claude → OpenAI 兼容 → 方舟）': ['Auto (Claude → OpenAI-compatible → Ark)', '自動（Claude → OpenAI互換 → Ark）'],
  '🧠 Hermes 门户（本机零 Key）': ['🧠 Hermes portal (local, zero keys)', '🧠 Hermes ポータル（ローカル・キー不要）'],
  'OpenAI 兼容（自定义 / BYO-LLM）': ['OpenAI-compatible (custom / BYO-LLM)', 'OpenAI 互換（カスタム / BYO-LLM）'],
  'Anthropic Claude（$20 硬顶账本）': ['Anthropic Claude ($20 hard-cap ledger)', 'Anthropic Claude（$20 上限）'],
  '方舟 Ark': ['Volc Ark', '方舟 Ark'],
  '▶ 启动门户': ['▶ Start portal', '▶ ポータル起動'],
  '✅ 测试当前通道': ['✅ Test this channel', '✅ この経路をテスト'],
  '一键预设：': ['Presets:', 'プリセット：'],
  '不带 /v1，如 http://127.0.0.1:11434': ['without /v1, e.g. http://127.0.0.1:11434', '/v1 なし。例 http://127.0.0.1:11434'],
  ...
  // ===== 2026-08-28 🧩 ComfyUI · MiniMax H3 =====
  '本机免开 UI 出片': ['local render, no ComfyUI window needed', 'ローカル生成（ComfyUI の画面不要）'],
  '🧩 本机出片（ComfyUI 自动运行）': ['🧩 Render locally (ComfyUI runs itself)', '🧩 ローカル生成（ComfyUI 自動起動）'],
